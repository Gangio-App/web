...
export function TextEmbed(props: { embed: TextEmbedClass | WebsiteEmbed }) {
  ...
  /**
   * Determine whether this is a "rich" special embed that shows its own internal metadata
   */
  const isSpecial = () =>
  ...
  return (
    <Base style={{ "border-color": props.embed.colour }}>
      <Content gap={isSpecial() ? "sm" : "md"} grow>
        <Show
          when={
            props.embed.type === "Website" &&
        ...
        <Show when={props.embed.title && !isSpecial()}>
        ...
        <Show when={props.embed.description && !isSpecial()}>
          <Description>
        ...
        <Show when={props.embed.type === "Website"}>
          <Switch>
            ...
            <Match
              when={
                (props.embed as WebsiteEmbed).image?.size === "Large" &&
                !isSpecial()
              }
            >
              <SizedContent
