...
export function UserCard(
  ...
  const voiceRoom = createMemo(() => {
    ...
    // Check privacy and relationship requirements first
    const showActivity = state.settings.getValue("privacy:show_voice_activity");
    const isFriend = props.user.relationship === "Friend";
    const hasMutualServer = !!client().servers.find((s) =>
      s.members.has(recipientId),
    );

    if (!showActivity || !isFriend || !hasMutualServer) return;
