...
export default function ServerOverview(props: ServerSettingsProps) {
  ...
  const canEditDiscoverable = () =>
    client().user?.id === props.server.ownerId;
  const canManageServer = () =>
    client().user?.id === props.server.ownerId ||
    props.server.havePermission("ManageServer");
  const canManageCustomisation = () =>
    client().user?.id === props.server.ownerId ||
  ...
  createEffect(
    on(
      ...
      (discoverable) =>
        !editGroup.controls.discoverable.isDirty &&
        editGroup.controls.discoverable.setValue(discoverable ?? false),
      { defer: true },
    ),
  );
  ...
  createEffect(
    on(
      canManageServer,
      (canEdit) => {
        editGroup.controls.name.markDisabled(!canEdit);
        editGroup.controls.description.markDisabled(!canEdit);
        editGroup.controls.sys_user_joined.markDisabled(!canEdit);
  ...
  createEffect(
    on(
      canManageCustomisation,
      (canEdit) => {
        editGroup.controls.icon.markDisabled(!canEdit);
        editGroup.controls.banner.markDisabled(!canEdit);
      },
      { defer: true },
    ),
  );
  ...
  async function onSubmit() {
    ...
    if (editGroup.controls.discoverable.isDirty && canEditDiscoverable()) {
      changes.discoverable = editGroup.controls.discoverable.value;
    }
    ...
    <Column gap="xl">
      <form onSubmit={submit}>
        <Column>
          ...
          <Column>
            <Form2.Checkbox control={editGroup.controls.discoverable}>
              <Trans>Allow this server to appear in Discover</Trans>
            </Form2.Checkbox>
            <Show when={!canEditDiscoverable()}>
              <Text class="label">
                <Trans>Only server owners with privileged access can change this setting.</Trans>
              </Text>
            </Show>
          </Column>
