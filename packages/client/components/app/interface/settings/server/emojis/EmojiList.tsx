...
export function EmojiList(props: { server: Server }) {
  ...
  async function onSubmit() {
    ...
    if (!res.ok) {
      throw new Error(`Failed to upload emoji: ${res.statusText}`);
    }

    const data: { id: string } = await res.json();

    if (!data.id) {
      console.error("Upload response missing ID:", data);
      throw new Error("Failed to upload emoji: No ID returned from server");
    }
