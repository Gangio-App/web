...
export function ViewBot(props: { bot: Bot }) {
  ...
  const [isSubmittingToDiscover, setIsSubmittingToDiscover] =
    createSignal(false);

  async function submitToDiscover() {
    if (props.bot.discoverable) return;
    if (isSubmittingToDiscover()) return;

    setIsSubmittingToDiscover(true);
    try {
      if (!props.bot.public) {
        await props.bot.edit({ public: true });
      }
      await props.bot.edit({ discoverable: true });
    } catch (err) {
      showError(err);
    } finally {
      setIsSubmittingToDiscover(false);
    }
  }
    ...
    <Column gap="lg">
      ...
      <CategoryButton.Group>
        ...
        <CategoryButton
          ...
          action="chevron"
          onClick={submitToDiscover}
          disabled={props.bot.discoverable || isSubmittingToDiscover()}
        >
