...
function brandSessionName(name?: string) {
  if (!name) return name;
  return name.replace(/^stoat\b/i, "Gangio");
}
  ...
  return (
    <CategoryButtonGroup>
      <CategoryCollapse
        ...
        description={brandSessionName(currentSession()?.name)}
          ...
          <For each={props.otherSessions()}>
            {(session) => (
              <CategoryCollapse
                ...
                title={<Capitalise>{brandSessionName(session.name)}</Capitalise>}
                description={
                  <Trans>
