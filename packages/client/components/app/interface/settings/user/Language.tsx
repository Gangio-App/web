...
function PickDateFormat() {
  ...
  return (
    <CategoryCollapse
      ...
      title={<Trans>Select date format</Trans>}
      description={
        date() === "DD/MM/YYYY"
    ...
    >
      <CategoryButton
        icon={"blank"}
        onClick={() => state.locale.resetDateFormat()}
        description={<Trans>Reset date format</Trans>}
      >
        <Trans>Default</Trans>
      </CategoryButton>
      <CategoryButton
...
function PickTimeFormat() {
  ...
  return (
    <CategoryCollapse
      ...
      title={<Trans>Select time format</Trans>}
      description={time() === "HH:mm" ? t`24 hours` : t`12 hours`}
    >
      <CategoryButton
        icon={"blank"}
        onClick={() => state.locale.resetTimeFormat()}
        description={<Trans>Reset time format</Trans>}
      >
        <Trans>Default</Trans>
      </CategoryButton>
      <CategoryButton
...
function ConfigureRTL() {
  ...
  const currentLanguage = () => Languages[i18n().locale as never] as LanguageEntry;
  const isRTL = () => state.get("locale").options.rtl ?? currentLanguage().rtl;

  return (
    <CategoryButton
      icon={<MdTranslate {...iconSize(22)} />}
      description={<Trans>Flip the user interface right to left</Trans>}
      action={<Checkbox checked={isRTL()} />}
      onClick={() => state.locale.setRTL(!isRTL())}
    >
...
function ContributeLanguageLink() {
  return (
    <a href="https://gangio.pro/language/contribution/" target="_blank">
      <CategoryButton
        action="external"
        icon={<MdTranslate {...iconSize(22)} />}
        onClick={() => void 0}
