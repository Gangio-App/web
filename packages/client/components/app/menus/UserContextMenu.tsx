...
export function UserContextMenu(props: {
  ...
  function openDm() {
    props.user
      .openDM()
      .then((channel) => navigate(channel.path ?? `/channel/${channel.id}`));
  }
  ...
  function inviteBot() {
    client()
      ...
      .api.get(`/bots/${props.user.id as ""}/invite`)
  ...
  return (
    <ContextMenu class="UserContextMenu">
      ...
      <Show when={props.user.bot && !props.user.self}>
        <ContextMenuButton icon={MdAssignmentInd} onClick={inviteBot}>
          <Trans>Invite bot to server</Trans>
        </ContextMenuButton>
        <ContextMenuDivider />
      </Show>
