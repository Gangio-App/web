...
import MdInbox from "@material-design-icons/svg/filled/inbox.svg?component-solid";
...
export const ServerList = (props: Props) => {
  ...
  const inboxState = createMemo(() => {
    let mentions = 0;
    let unread = false;
    for (const channel of client().channels.toList()) {
      const channelMentions = channel.mentions?.size ?? 0;
      mentions += channelMentions;

      if (!unread) {
        if (!state.notifications.isMuted(channel) && channel.unread) {
          const config = state.notifications.computeForChannel(channel);
          if (config !== "none") {
            if (config === "mention") {
              if (channelMentions > 0) unread = true;
            } else {
              unread = true;
            }
          }
    ...
    const hasReward = client().user && (client().user!.badges ?? 0) & 256 && !localStorage.getItem("reward_early_adopter_seen");
    const rewards = hasReward ? 1 : 0;

    return { mentions, rewards, unread: unread || mentions > 0 || hasReward, total: mentions + rewards };
  });
  ...
  return (
    <ServerListBase>
      ...
      <Tooltip placement="right" content={t`Inbox`}>
        <a
          class={entryContainer({
            indicator: inboxState().unread ? "alert" : undefined,
          })}
        ...
        >
          <Avatar
            size={42}
            fallback={<MdInbox />}
            holepunch={inboxState().unread ? "top-right" : undefined}
            overlay={
              <Show when={inboxState().unread}>
                <Unreads.Graphic unread count={inboxState().total} />
              </Show>
            }
            interactive
          />
